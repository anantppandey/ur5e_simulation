...
1. Connect Isaac Sim to VS Code (see Isaac Sim section below).
2. Enable the Isaac Sim VS Code extension inside Isaac Sim.
3. Configure the VS Code extension settings (see screenshots below).
4. After setup, run the example scripts under `Isaac_sim/Trajectory Follower` and `Isaac_sim/End Effector Teleop`.

## Isaac Sim — connect to VS Code

Follow these quick steps to establish the VS Code <-> Isaac Sim connection.

1. Install and enable the Isaac Sim extension in Isaac Sim. See the extension UI here:

![Isaac Sim extension](/home/anant/git/ur5e_simulation/assets/isaac_sim_extension.png)

     - Make sure you enable the extension inside Isaac Sim (Extension must be turned on from the Isaac Sim Extensions window).

2. Configure the extension settings in VS Code. Open your VS Code settings and set the required fields.

![VS Code extension settings](/home/anant/git/ur5e_simulation/assets/vs_code_extension_settings.png)

Tip: you can find the connection values from Isaac Sim by going to: Isaac Sim > Windows > VS Code — see the values UI here:

![Isaac Sim > Windows > VS Code values](/home/anant/git/ur5e_simulation/assets/isaac_sim_extension_2.png)
...
- `Isaac_sim/End Effector Teleop/bot_spawner.py`
  ...
  - Sets up a pick-and-place task and controller for a UR manipulator (adds the task to the world, initializes the PickPlaceController, and runs the simulation loop to apply controller actions).
...
That's the first set of updates — I read the scripts to extract the summaries above. Tell me what you'd like next (expand any README sections, add run instructions, or add quick start commands / example commands to invoke the Python scripts).
